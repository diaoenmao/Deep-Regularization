...
class ADMM_Adam_Layer(Optimizer):
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for w, vk_temp, yk_temp, zk_temp, wk_temp, wanda_temp in zip(group['params'], self.vk, self.yk,
                ...
                ck = torch.norm(wanda_temp * zk_temp, p=1)  #相比之前没score的在这里进行了修改，需要乘以scorebased矩阵，下面同理
                dk = qk + vk_temp / p
                yita = torch.norm(wanda_temp * dk, p=2)#相比之前没score的在这里进行了修改，需要乘以scorebased矩阵，下面同理
                miu = self.C * ck / self.N
                D_k = (miu * torch.mul(wanda_temp, wanda_temp)) / (p * (yita) ** 3)
...
for exp in range(expriment_num):
    ...
    for epoch in range(epochs):
        ...
        for data in dataloader_train:  # [data1,data2,.,.,data100]  [ [a1,(b1,c1)], [a2,(b2,c2)]  for _,(_,label) in enu
            ...
            optimizer = ADMM_Adam_Layer(params, lr=lr, N=600, C=C, vk=vk, wk=wk, yk=yk, zk=zk, score=scores_list)
            optimizer.zero_grad()
...
SAVE_DIR = 'results'
OPTIMIZER_TYPE = 'ADMM_layer_magnitude'
MODEL_TYPE = 'cnn3_CIFAR10'
