...
def load_dataset(name, seed=SEED):
    ...
    if name in {'fashion', 'mnist'}:
        ...
        y_train = np.loadtxt(os.path.join(DATA_PATH, 'UCI HAR Dataset', 'train', 'y_train.txt')).astype(int) - 1
        X_test = np.loadtxt(os.path.join(DATA_PATH, 'UCI HAR Dataset', 'test', 'X_test.txt'))
        y_test = np.loadtxt(os.path.join(DATA_PATH, 'UCI HAR Dataset', 'test', 'y_test.txt')).astype(int) - 1
    else:
