...
from src.core import run_fa_method, run_fs_method
...
def run_method(
    splits, X, X_tilde, y, method_name, dataset_name, k, k2=None, h_scores=None
):
    ...
    for i in range(len(splits)):
        ...
        if scores is not None:
            ...
            best_2k.append(
                np.sum([i in correct_indices for i in indices[-2 * k :]]) / k
            )
            if k2 is not None:
                indices = np.argsort(np.abs(scores))
                best_k2.append(
                    np.sum([i in correct_indices2 for i in indices[-k2:]]) / k2
                )
                indices = np.argsort(np.abs(scores2))
                best_2k2.append(
                    np.sum([i in correct_indices2 for i in indices[-2 * k2 :]]) / k2
                )
...
def process_dataset_(method_name, dataset_name, k, ns, n_samples=1000):
    ...
    with open(
        os.path.join(RESULTS_PATH, f"{method_name}-{dataset_name}-{n_samples}.txt"), "w"
    ) as f:
        ...
        for n_features in ns:
            print(
                f'Running "{method_name}" with {n_features} features on "{dataset_name}".'
            )
            ...
            (
                ...
                aurocs,
                auprcs,
            ) = run_method(splits, X, X_tilde, y, method_name, dataset_name, k, k2=k2)
...
def process_dataset_with_fa_methods(
    dataset_name, k, ns, n_samples=1000, bootstrap=False, with_train=False
):
    splits = list(KFold(n_splits=6).split(np.random.rand(n_samples)))

    models = {
        (n_features, j): NNwrapper.create(dataset_name, n_features, 2)
        for n_features in ns
        for j in range(6)
    }
    for j in range(6):
            ...
            for n_features in ns:
                ...
                for j in range(len(splits)):
                    ...
                    for wrapper in models[(n_features, j)]:
                        if not wrapper.trained:
                            if bootstrap:
                                idx = np.random.randint(
                                    0, len(X_train), size=int(0.8 * len(X_train))
                                )
                        ...
                        if with_train:
                            hr_scores.append(
                                run_fa_method(wrapper, X_train, method_name)
                            )
                        else:
                            hr_scores.append(
                                run_fa_method(wrapper, X_test, method_name)
                            )
