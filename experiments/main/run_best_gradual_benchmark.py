# -*- coding: utf-8 -*-
"""
Run Full Benchmark with Best Gradual ADMM Configuration.

Uses the best configuration discovered in Experiment A:
...
This produces the main results for Table 1.
"""
...
def run_single_fold(
    ...
    """Run one fold of gradual ADMM with best configuration."""
...
def run_benchmark():
    """Run full benchmark with best gradual configuration."""
    print("=" * 70)
    print("BEST GRADUAL ADMM BENCHMARK")
    ...
    # Best training configuration (from Experiment A)
    ...
    for dataset in datasets:
        ...
        for fold_idx, (train_idx, test_idx) in enumerate(skf.split(dataset["X"], dataset["y"])):
            ...
            print(f"  best-k: {fold_result['best_k']:.4f}")
        ...
        print(f"\n{dataset['name']} Summary: mean_best-k = {mean_best_k:.4f} ± {std_best_k:.4f}")

    # Compute overall mean
    overall_mean = np.mean([results[ds]["mean_best_k"] for ds in results.keys()])
    print(f"\n{'='*70}")
    print("FINAL RESULTS")
    print(f"{'='*70}")
    print(f"Overall Mean best-k: {overall_mean:.4f}")

    # Print comparison table
    print("\nComparison with baseline SADMM-FS:")
    print("Method              | XOR    | Ring   | Ring+XOR | Ring+XOR+Sum | Mean")
    print("-" * 70)
    print(f"Gradual+Soft_no_rw  | {results['xor']['mean_best_k']:.2f}   | {results['ring']['mean_best_k']:.2f}   | {results['ring+xor']['mean_best_k']:.2f}     | {results['ring+xor+sum']['mean_best_k']:.2f}       | {overall_mean:.4f}")
    print(f"Baseline SADMM-FS   | 1.00   | 0.50   | 0.62     | 0.67         | 0.6278")
    ...
    output_file = output_dir / f"best_gradual_benchmark_{timestamp}.json"
    ...
    with open(output_file, "w") as f:
        json.dump({
            ...
            "results": results,
            "overall_mean_best_k": overall_mean,
        }, f, indent=2)
