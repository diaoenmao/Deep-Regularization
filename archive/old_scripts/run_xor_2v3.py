...
# NOTE: Fixed 2025-02-25 to use canonical src/data.py. Results need re-running.
from src.data import generate_xor_dataset

N_SEEDS = 3

def generate_xor(m=128, n=1000, seed=42):
    np.random.seed(seed)
    X, y = generate_xor_dataset(n, m)
    return X, y
...
def run_xor(three_variable, seed, m=128):
    ...
    # Standardise features (z-score) — matches the main benchmark pipeline
    scaler = _Scaler()
