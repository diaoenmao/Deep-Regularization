...
class GaussianNoise(torch.nn.Module):
    ...
    def forward(self, X):
        if self.training:
            X = X + self.stddev * torch.randn_like(X)
        return X


# ---------------------------------------------------------------------------
# Model preset configurations for architecture comparison
# ---------------------------------------------------------------------------

MODEL_PRESETS = {
    # Small model: LassoNet-style (2 layers, 32 units)
    "small_2layer": {
        "n_hidden_layers": 2,
        "latent_size": 32,
        "gaussian_noise": 0.0,
    ...
    # Small model with noise variant
    "small_2layer_noise": {
        "n_hidden_layers": 2,
        "latent_size": 32,
    ...
    # Medium model: Compromise between small and baseline (3 layers, 48 units)
    "medium_3layer": {
        "n_hidden_layers": 3,
        "latent_size": 48,
        "gaussian_noise": 0.0,
    ...
    # Baseline model: Current standard (5 layers, 58 units)
    "baseline_5layer": {
        "n_hidden_layers": 5,
        "latent_size": 58,
...
def create_model_with_preset(
    ...
    Args:
        ...
        preset: Preset name ('small_2layer', 'medium_3layer', 'baseline_5layer')
        **overrides: Override any preset configuration values

    Returns:
        Model instance with specified configuration

    Examples:
        # Create baseline model
        model = create_model_with_preset(128, 2, preset="baseline_5layer")

        # Create medium model with custom dropout
    ...
    return Model(input_size, n_classes, **config)
