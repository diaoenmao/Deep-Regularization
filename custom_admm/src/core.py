...
def run_fs_method(
    ...
    if method_name == "random":
        ...
        # Train separate models for k and 2k to ensure fair best-k evaluation
        n_selected_k = min(k, n_features)
        n_selected_2k = min(2 * k, n_features)

        # Model for k features (used for best-k evaluation)
        fsnet_k = FSNet(
            Model(n_selected_k, n_classes), n_features, 30, n_selected_k, n_classes
        )
        fsnet_k.fit(X_train, y_train)
        scores = fsnet_k.get_feature_importances()

        # Model for 2k features (used for best-2k evaluation and predictions)
        if n_selected_2k != n_selected_k:
            fsnet_2k = FSNet(
                Model(n_selected_2k, n_classes), n_features, 30, n_selected_2k, n_classes
            )
            fsnet_2k.fit(X_train, y_train)
            scores2 = fsnet_2k.get_feature_importances()
            y_train_hat = fsnet_2k.predict(X_train)
            y_hat = fsnet_2k.predict(X_test)
        else:
            scores2 = scores.copy()
            y_train_hat = fsnet_k.predict(X_train)
            y_hat = fsnet_k.predict(X_test)
    elif method_name == "admm_input_group":
    ...
    elif method_name == "stg":
        y_train_hat, y_hat, scores, scores2 = run_stg_fs(
            X_train.astype(np.float32),
            y_train,
            X_test.astype(np.float32),
            n_classes,
